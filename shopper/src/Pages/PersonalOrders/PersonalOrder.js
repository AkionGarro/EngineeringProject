	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				...
				const filteredUsers = userData.filter(user => user.email.toLowerCase().includes(searchQuery.toLowerCase()))
		...
		<>
			<Container className="container_personal">
				...
				{fields.map((field, index) => (
					<Grid container spacing={3} key={index} className="grid-container">
						...
						<Grid item xs={12} className="button-container">
							<DeleteIcon className="iconoEliminar" onClick={() => removeFields(index)}>
				...
				<div className="opciones-botones">
					<Button variant="contained" color="primary" onClick={addFields} className="add-button">
						+ Agregar otro producto
					</Button>
					<Button variant="contained" color="error" onClick={deleteAll} className="add-button">
						Vaciar pedido
					</Button>
				</div>
